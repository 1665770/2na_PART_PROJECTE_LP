...
void MapaSolucio::parsejaXmlElements(std::vector<XmlElement>& xmlElements)
{
	...
	for (int i = 0; i < xmlElements.size(); i++)
	{
		if (xmlElements[i].id_element == "node")
		{
			...
			if (hiHaNom && !esCami) // Els nodes que són Punt d’Interés són nodes que no són de camí, i que tenen un tag amb un atribut name
			{
		...
		{
			bool highway = false;
			std::vector<std::string> nodes; //aquí guardem les "ref" dels "nd" dels camins
			...
			for (int j = 0; j < xmlElements[i].fills.size(); j++)
			{
				if (xmlElements[i].fills[j].first == "tag")
				{
					if (xmlElements[i].fills[j].second[0].second == "highway") // si no és highway no el tenim en compta
						highway = true;
				}
			...
			if (highway) //si és highway afegim al vector de camins les coordenades dels nodes que encara hem de trobar
			{
				int j = 0;
					...
					if (xmlElements[j].id_element == "node")
					{
						...
						for (int k = 0; k < xmlElements[j].atributs.size(); k++)
						{
							if (xmlElements[j].atributs[k].first == "id")
							{
								...
								{
									nodeTrobat = true;
									nodes.erase(nodes.begin()); //elimina el node del vector de nodes per buscar perquè ja l'hem trobat
								}
							}
